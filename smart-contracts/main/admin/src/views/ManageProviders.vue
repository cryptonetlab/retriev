    ...
    <div class="container">
      <div class="row">
        <div class="col-12 col-md-8 col-lg-8 pt-5 text-center">
          ...
          <div v-if="account">
            <h3 class="mb-5">Fix provider status</h3>
            <select v-model="status">
              <option :value="true">ACTIVE</option>
              <option :value="false">NOT ACTIVE</option></select
            ><br /><br />
            <input
              class="text-center"
              placeholder="Address"
            ...
            <input
              ...
              v-model="endpoint"
            /><br /><br />
            <div class="btn mint-btn" @click="fixState()">Fix state</div>
          </div>
        </div>
      ...
      if (app.endpoint !== "" && app.address !== "") {
        try {
          ...
          const receipt = await contractInstance.methods
            .setProviderStatus(app.address, app.status, app.endpoint)
            .send({ from: app.account })
            .on("transactionHash", (tx) => {
              app.printLog("Waiting for confirmation at: " + tx);
            });
          app.printLog("Status updated correctly");
