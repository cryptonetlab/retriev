...
async function processRounds() {
    ...
    for (let k in configs.referees) {
        ...
        // Run double check
        const verified = await refereeContract.verifyRefereeSignature(signature, deal_index, refereeWallet.address)
        if (verified) {
        ...
        if (parsedSignaturesCount >= threshold && !slashed) {
            ...
            try {
                const slashTransaction = await refereeContract.processAppeal(deal_index, referees_addresses, referees_signatures)
